...
function switchView(viewName) {
    ...
    // 2. Xóa trạng thái hiển thị (active) của cả 2 bên
    ...
    if (viewName === "client") {
        ...
        // 1. Gọi hàm hiển thị lịch sử bảng hóa đơn cũ của bạn
        renderBookingHistory();
        // 2. IN THỐNG KÊ (ADM-01): Gọi các hàm hiện đại vừa viết và in ra Console để kiểm tra
        const stats = hotelSystem.getAdminStatistics();
    ...
    if (viewName === "client") {
        ...
        // Bọc try-catch để nếu chưa có dữ liệu đặt phòng, biểu đồ không làm sập trang admin
        try {
            drawRevenueChart();
        } catch (e) {
...
function renderBookingHistory(sortedBookings = null) {
    ...
    // Dùng toán tử Spread [...] để tạo bản sao, tránh làm đảo lộn mảng gốc của hệ thống
